# Information Architecture — anagha-todalbagi.github.io

## Format

Single page, one column, five numbered sections plus hero and footer. No nav bar — a small fixed mono "CONTACT" link top-right. The takeaway is available at every glance:

- **Glance 1 (0–10 s):** monogram + name + "Product Leader · Deep Tech at Scale · AI-Native" + the positioning lede + three stats. Who she is, with numbers.
- **Glance 2 (one scroll):** how she works, then shipped things with proof links. Why to believe her.
- **Glance 3:** the career ledger — Suki (seed, 5th hire) → Duetto → Commure → Microsoft. The arc.
...
### 05 — Beyond `SAN FRANCISCO`
Books and Rec. SF book club · tennis, reading, first-principles thinking, teams across six countries · the guide's closing quote as a blockquote: *"Make the best use of what's in your power, and take the rest as it happens."*

### Footer (graphite statement surface)
Reversed monogram · "Say hello." · one warm sentence · email + LinkedIn · colophon: `ANAGHA TODALBAGI · SAN FRANCISCO` / `BUILT FROM FIRST PRINCIPLES. MADE TO INCLUDE.`

## Cross-cutting

- Every claim resolves to a link, number, date, or named program.
- The page must read perfectly with images and motion stripped, and land its story with all text blurred. Both tests run before ship.
- Print = one-page resume with URLs expanded.
